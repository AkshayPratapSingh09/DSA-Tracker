const defaultProblems = [
        ...
        function openSettingsModal() {
            ...
            document.getElementById('gh-path').value = localStorage.getItem('ghPath') || 'dsa_tracker_data.json';
        ...
        async function fetchFromGitHub() {
            const token = localStorage.getItem('ghToken'), owner = localStorage.getItem('ghOwner'), repo = localStorage.getItem('ghRepo'), path = localStorage.getItem('ghPath');
            if (!token || !owner || !repo) return showToast("Configure settings first!");
            const icon = document.getElementById('settings-icon');
            icon.classList.add('animate-spin-fast');
            try {
                const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, { headers: { 'Authorization': `token ${token}` } });
                if (!response.ok) throw new Error("File not found");
                const data = await response.json();
                const content = JSON.parse(atob(data.content));
                if (content.problems) { mergeCloudData(content); showToast("Sync Complete"); }
        ...
        function mergeCloudData(cloud) {
            ...
            cloud.problems.forEach(cp => {
                const key = `${cp.topic.toLowerCase()}|${cp.title.toLowerCase()}`;
                if (!existingKeys.has(key)) { dsaProblems.push(cp); existingKeys.add(key); }
            });
            completionData = { ...completionData, ...cloud.completion };
            dailyTargets = { ...dailyTargets, ...cloud.targets };
        ...
        async function pushToGitHub() {
            ...
            try {
                let sha = null;
                const getFile = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, { headers: { 'Authorization': `token ${token}` } });
                if (getFile.ok) { sha = (await getFile.json()).sha; }
                const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, {
                    method: 'PUT', headers: { 'Authorization': `token ${token}` },
                    body: JSON.stringify({ message: "Update DSA Sync", content: btoa(JSON.stringify({ problems: dsaProblems, completion: completionData, targets: dailyTargets }, null, 2)), sha })
                });
                if (response.ok) showToast("Backup Successful!"); else throw new Error("Push failed");
            } catch (err) { showToast("Error: " + err.message); } finally { icon.classList.remove('animate-spin-fast'); }
        }
